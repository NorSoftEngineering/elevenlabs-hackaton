...
begin
  ...
  -- Check if candidate is already added and lock the row if it exists
  perform 1 
  from interviews_candidates ic
  where ic.interview_id = v_invitation.interview_id
  and ic.candidate_id = p_candidate_id
  for update;

  -- If candidate not already added, insert them
  if not found then
