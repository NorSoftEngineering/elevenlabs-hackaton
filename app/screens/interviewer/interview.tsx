...
import { type ActionFunctionArgs, type LoaderFunctionArgs, useLoaderData, useNavigation, useSubmit, Form, redirect } from 'react-router';
...
function InterviewStatus({ status, onUpdate }: { status: InterviewWithRelations['status']; onUpdate?: (status: InterviewWithRelations['status']) => void }) {
  ...
  if (!onUpdate) {
    return (
      <span className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-sm font-medium ${style.bg} ${style.text}`}>
        {style.icon}
            ...
            <div>
              <section className="mb-6">
                ...
                <div className="space-y-4">
                  <div>
                    <h3 className="text-sm font-medium text-gray-600 mb-2">Accepted ({interview.candidates?.length || 0})</h3>
                    <div className="space-y-2">
